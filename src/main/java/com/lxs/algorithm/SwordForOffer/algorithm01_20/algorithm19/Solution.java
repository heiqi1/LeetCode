...
2. 如果需要匹配的字符串没有遍历完，模式字符串已经匹配完，后面的字符串不能匹配，那么就是不匹配
3. 如果不是上面两个情况，那么首先看一下模式串当前字符下一个字符是*，
a. 如果下一个字符是*且当前需要匹配的字符和模式当前字符匹配（相同或者为.）
...
b. 如果当前字符不匹配，那么就使用下一个模式匹配当前字符，即（strIndex,patternIndex+2）
4. 如果模式字符下一个不是*，那么就看当前字符和当前模式是否匹配，如果匹配，那么就行判断下一个字符和后面的模式是否匹配，即（strIndex+1,patternIndex+1）。否则就是不匹配的。
*/
